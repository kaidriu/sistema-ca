...
//PARA BUSCAR LAS NOTAS DE CREDITO
	...
	if($action == 'ajax'){
		...
		$g = mysqli_real_escape_string($con,(strip_tags($_REQUEST['g'], ENT_QUOTES)));
		$ordenado = "fecha_gr";//mysqli_real_escape_string($con,(strip_tags($_GET['ordenado'], ENT_QUOTES)));
		...
		$datos_clientes = $con->query($sql_clientes);
			...
			$sql_facturas_contar = "SELECT * FROM encabezado_gr WHERE id_cliente ='". $id_cliente_contar ."' ";
			$datos_facturas_contar = $con->query($sql_facturas_contar);
			while ($row_facturas_contar=mysqli_fetch_array($datos_facturas_contar)){
			$id_encabezado_factura_contar[]=$row_facturas_contar['id_encabezado_gr'];
			}
			}
		...
		$id_cliente=intval($row_clientes['id']);
		 ...
		 $aColumns = array('secuencial_gr','serie_gr','factura_aplica','nombre','nombre_comercial');//Columnas de busqueda
		 $sTable = "encabezado_gr, empresas";
		 $sWhere = "WHERE id_cliente ='". $id_cliente ."' and encabezado_gr.ruc_empresa=empresas.ruc";
		if ( $_GET['g'] != "" )
		{
			$sWhere = "WHERE (id_cliente ='". $id_cliente ."' and encabezado_gr.ruc_empresa=empresas.ruc AND ";
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				$sWhere .= $aColumns[$i]." LIKE '%".$g."%' AND id_cliente ='". $id_cliente ."' and encabezado_gr.ruc_empresa=empresas.ruc OR ";
			}
			$sWhere = substr_replace( $sWhere, "AND id_cliente ='". $id_cliente ."' and encabezado_gr.ruc_empresa=empresas.ruc ", -3 );
			$sWhere .= ')';
		}
		...
		$id_encabezado_factura[]=$row_facturas['id_encabezado_gr'];
		}
		}
		...
		?>
			...
			<div class="table-responsive">
			  <table class="table table-hover">
				...
				<th >Fecha</th>
				<th >Emisor</th>
				<th >Número Guía Remisión</th>
				<th >Número factura aplica</th>																		
				<th >Estado</th>								
		...
		$sql_facturas_encontradas = "SELECT * FROM encabezado_gr ef, empresas em WHERE ef.id_encabezado_gr='". $id_encabezado_factura[$i] ."' and ef.ruc_empresa=em.ruc ";
		$query = mysqli_query($con, $sql_facturas_encontradas);
				
		while ($row_facturas_encontradas=mysqli_fetch_array($query)){
		$id_encabezado=$row_facturas_encontradas['id_encabezado_gr'];
		$fecha_factura=$row_facturas_encontradas['fecha_gr'];
		$serie_factura=$row_facturas_encontradas['serie_gr'];
		$secuencial_factura=$row_facturas_encontradas['secuencial_gr'];
		$nombre_cliente_factura=$row_facturas_encontradas['nombre'];
		$estado_sri=$row_facturas_encontradas['estado_sri'];
		$factura_modificada=$row_facturas_encontradas['factura_aplica'];
					...
					case "ANULADA":
						...
						$estado='GUÍA DE REMISIÓN ANULADA';
						break;
					case "NO APLICA":
		...
		<td class='col-md-2'><span class="pull-right">
			<a href="../sistema/ajax/imprime_documento.php?id_documento=<?php echo base64_encode($id_encabezado) ?>&tipo_documento=gr&tipo_archivo=pdf" class='btn btn-default btn-xs' title='Ver'>Pdf</i> </a>
			<a href="../sistema/ajax/imprime_documento.php?id_documento=<?php echo base64_encode($id_encabezado) ?>&tipo_documento=gr&tipo_archivo=xml" class='btn btn-default btn-xs' title='Ver'>Xml</i> </a>
		</td>
		</tr>

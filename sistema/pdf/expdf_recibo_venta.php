...
if($action == "recibo_venta_a4"){
	...
	if($action == "nota_entrega"){
		...
		$pdf->Ln();
		$pdf->SetX(10);
		$pdf->SetWidths(array(30,90,20,25,25));
		$pdf->Row_tabla(array(utf8_decode('Código'),'Detalle','Cantidad','Valor unitario','Subtotal'));
		$sutotal_a_pagar=array();
		$iva=array();
		$total_iva=0;
		...
		$total_iva = array_sum($iva);
			$sutotal_a_pagar[] = (($row_detalle['cantidad']*$row_detalle['valor_unitario'])-$row_detalle['descuento']);
			$pdf->SetX(10);
			$pdf->Row_tabla(array($row_detalle['codigo_auxiliar'], utf8_decode($row_detalle['nombre_producto']),number_format($row_detalle['cantidad'],2,'.',''),number_format($row_detalle['valor_unitario'],2,'.',''), number_format(abs($row_detalle['valor_unitario']*$row_detalle['cantidad']-$row_detalle['descuento']),2,'.','')));
			}

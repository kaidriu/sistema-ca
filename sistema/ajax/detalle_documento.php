...
?>
	...
	<div class="row">
		...
		$subtotal_general = 0;
		$total_descuento = 0;
		$sql_factura = mysqli_query($con, "select sum(round(subtotal - descuento,2)) as subtotal_general, sum(round(descuento,2)) as total_descuento  FROM cuerpo_recibo WHERE id_encabezado_recibo='" . $id_encabezado_recibo . "' group by tarifa_iva ");
		$row_subtotal = mysqli_fetch_array($sql_factura);
		$subtotal_general = $row_subtotal['subtotal_general'];
		$total_descuento = $row_subtotal['total_descuento'];
		?>
					...
					<table class="table">
						...
						$total_iva =0;
							if ($impuestos_recibo=='2'){
								$detalle_recibo = mysqli_query($con, "SELECT * FROM cuerpo_recibo WHERE id_encabezado_recibo = '".$id_encabezado_recibo."' ");
								...
								$iva[] = number_format((($row_detalle['cantidad']*$row_detalle['valor_unitario'])-$row_detalle['descuento']) * ($porcentaje_iva/100), 2, '.', '');
							...
							<td class="text-right" style="padding: 2px;"><?php echo number_format($total_iva, 2, '.', ''); ?></td>
							<td style="padding: 2px;"></td>
							<td style="padding: 2px;"></td>	
						</tr>
							<?php
...
if ($action == 'facturas_ventas') {
	...
	ef.tasa_turistica as tasa, ef.aut_sri as aut_sri, ef.estado_sri as estado_sri, cl.ruc as ruc_cliente 
	FROM encabezado_factura as ef INNER JOIN clientes as cl ON cl.id=ef.id_cliente WHERE ef.id_encabezado_factura = '" . $id_encabezado_factura . "' ";
	$query_encabezado = mysqli_query($con, $sql_encabezado);
...
?>
	<div style="padding: 0px; margin-bottom: 5px; margin-top: -10px;" class="alert alert-info" role="alert">
		<div class="panel-heading" style="padding: 4px;">
			...
			<?php
				...
				?>
				<a href="http://64.225.69.65:8000/facturas_autorizadas/<?php echo $ruc_empresa ?>/<?php echo $ruc_cliente ?>/FAC<?php echo $factura ?>.pdf" class='btn btn-default btn-sm' title='Descargar' target="_blank" download>Pdf</i> </a>
				<a href="http://64.225.69.65:8000/facturas_autorizadas/<?php echo $ruc_empresa ?>/<?php echo $ruc_cliente ?>/FAC<?php echo $factura ?>.xml" class='btn btn-default btn-sm' title='Descargar' target="_blank" download>Xml</i> </a>
				<?php
					}
				?>
	...
	<div class="row">
		...
		$subtotal_general = 0;
		$total_descuento = 0;
		$sql_factura = mysqli_query($con, "select sum(round(subtotal_factura - descuento,2)) as subtotal_general, sum(round(descuento,2)) as total_descuento  FROM cuerpo_factura WHERE ruc_empresa='" . $ruc_empresa . "' and serie_factura = '" . $ser_factura . "' and secuencial_factura='" . $num_factura . "'");
		$row_subtotal = mysqli_fetch_array($sql_factura);
		$subtotal_general = $row_subtotal['subtotal_general'];
		$total_descuento = $row_subtotal['total_descuento'];
		?>
			...
			<div class="panel panel-info">
				<div class="table-responsive">
					<table class="table">
						...
						$total_iva = 0;
						$subtotal_porcentaje_iva = 0;
						$sql = mysqli_query($con, "select ti.tarifa as tarifa, sum(round((((cf.cantidad_factura * cf.valor_unitario_factura) - descuento) * ti.tarifa /100),2))  as porcentaje FROM cuerpo_factura cf, tarifa_iva ti WHERE cf.ruc_empresa= '" . $ruc_empresa . "' and cf.serie_factura= '" . $ser_factura . "' and cf.secuencial_factura ='" . $num_factura . "' and ti.codigo = cf.tarifa_iva and ti.tarifa > 0 group by cf.tarifa_iva ");
						while ($row = mysqli_fetch_array($sql)) {
							$nombre_porcentaje_iva = strtoupper($row["tarifa"]);
							$porcentaje_iva = $row['porcentaje'];
							$subtotal_porcentaje_iva = $porcentaje_iva;
...
function detalle_asiento_contable($con, $ruc_empresa, $id_registro_contable)
{
	if ($id_registro_contable > 0) {
		$encabezado_diario = mysqli_query($con, "SELECT * FROM encabezado_diario WHERE ruc_empresa = '" . $ruc_empresa . "' and numero_asiento= '" . $id_registro_contable . "' ");
...
/*
		...
		if ($tipo_asiento == 'RETENCIONES_COMPRAS') {
									...
									<table class="table table-bordered">
										...
										?>
												<tr>
													...
													<td style="padding: 2px;"><a href="../pdf/pdf_diario_contable.php?action=diario_contable&id_diario=<?php echo $value['id_diario']; ?>" class='label label-default' title='Descargar asiento de <?php echo $value['tipo']; ?>' target="_blank"><i class="glyphicon glyphicon-print"></i> <?php echo $value['numero_asiento']; ?></a></td>
...
function busca_asiento_contable($con, $id_registro_contable, $ruc_empresa)
{
 ...
 WHERE enc_dia.ruc_empresa = '" . $ruc_empresa . "' and enc_dia.numero_asiento= '" . $id_registro_contable . "' order by det_dia.debe desc");
	...
	while ($row_detalle_contable = mysqli_fetch_array($detalle_contable)) {
		...
		$id_diario = $row_detalle_contable['id_diario'];
		$registro_contable[] = array('codigo_cuenta' => $codigo_cuenta, 'nombre_cuenta' => $nombre_cuenta, 'detalle_item' => $detalle_item, 'debe' => $debe, 'haber' => $haber, 'numero_asiento' => $numero_asiento, 'codigo_unico' => $codigo_unico, 'tipo' => $tipo, 'id_diario'=> $id_diario);
	}
...
function detalle_pago_compra($con, $ruc_empresa, $codigo_documento)
{
	...
	$id_comprobante = $row_documento['id_comprobante'];

	$detalle_pagos=mysqli_query($con, "SELECT det.codigo_documento as codigo_documento,
	det.valor_ing_egr as valor_ing_egr, det.numero_ing_egr as numero_ing_egr,
	enc.fecha_ing_egr as fecha_ing_egr FROM detalle_ingresos_egresos as det INNER JOIN ingresos_egresos as enc ON
	det.codigo_documento = enc.codigo_documento WHERE codigo_documento_cv = '".$codigo_documento."' and tipo_documento='EGRESO' " );
	$cuenta_registros=mysqli_num_rows($detalle_pagos);

	$detalle_nc = mysqli_query($con, "SELECT sum(total_compra) as total_compra FROM encabezado_compra WHERE factura_aplica_nc_nd = '" . $numero_documento . "' and mid(ruc_empresa,1,12) = '" . substr($ruc_empresa, 0, 12) . "' and id_comprobante='4' and id_proveedor='".$row_documento['id_proveedor']."' group by numero_documento");
...
?>
	<div class="row">
		<div class="panel-group" id="accordion_pago_compra" style="margin-bottom: 0px; margin-top: 0px;">
			<div class="col-xs-12">
				<div class="panel panel-info">
								...
								<div class="table-responsive">
									<table class="table table-bordered">
										...
										while ($row_detalle_pago = mysqli_fetch_array($detalle_pagos)) {
											...
											$fecha_emision=$row_detalle_pago['fecha_ing_egr'];
											...
											//$valor_del_documento = $row_forma_pago['valor_forma_pago'];
											...
											$cheque = $row_forma_pago['cheque']=='0'?"":$row_forma_pago['cheque'];
										...
										?>
										<tr class="info">
											<td style="padding: 2px; text-align: right;">Total pagos </td>

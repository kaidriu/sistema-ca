...
//PARA BUSCAR LOS ALUMNOS
	...
	if($action == 'buscar_alumnos'){
		// escaping, additionally removing everything that could be (html/javascript-) code
         $q = mysqli_real_escape_string($con,(strip_tags($_REQUEST['q'], ENT_QUOTES)));
		 $ordenado = mysqli_real_escape_string($con,(strip_tags($_GET['ordenado'], ENT_QUOTES)));
		 $por = mysqli_real_escape_string($con,(strip_tags($_GET['por'], ENT_QUOTES)));
		 $aColumns = array('al.cedula_alumno', 'al.nombres_apellidos','cl.nombre','cl.ruc', 'cl.email');//Columnas de busqueda
		...
		$sWhere = "WHERE al.ruc_empresa ='".  $ruc_empresa ." ' " ;//and match(al.nombres_alumno, al.apellidos_alumno) against('".$q."')
		if ( $_GET['q'] != "" )
		{
			...
			{
				$sWhere .= $aColumns[$i]." LIKE '".$like."%' and al.ruc_empresa = '".$ruc_empresa ."' OR ";
			}
		...
		$sql="SELECT al.id_alumno as id_alumno, al.tipo_id as tipo_id_alumno, al.cedula_alumno as cedula_alumno, nombres_apellidos as nombres_apellidos,
		...
		al.sucursal_alumno as sucursal_alumno, al.paralelo_alumno as paralelo_alumno, al.estado_alumno as estado_alumno, al.serie_facturar as serie_facturar, al.id_cliente as id_cliente,
		cl.tipo_id as tipo, cl.ruc as ruc_cliente, cl.nombre as nombre_cliente, cl.telefono as telefono_cliente, cl.direccion as direccion_cliente, 
		cl.plazo as plazo, cl.email as email FROM $sTable $sWhere LIMIT $offset, $per_page";
			...
			<div class="table-responsive">
			  <table class="table table-hover">
				<tr  class="info">
					...
					<th style ="padding: 0px;"><button style ="border-radius: 0px; border:0;" class="list-group-item list-group-item-info" onclick='ordenar("nombres_apellidos");'>Nombres y Apellidos</button></th>
				...
				while ($row=mysqli_fetch_array($query)){
					...
					<input type="hidden" value="<?php echo strtoupper($nombres_apellidos);?>" id="nombres_apellidos<?php echo $id_reg;?>">
					...
					<tr>
						...
						<td><a href="#" title='Editar datos alumno' onclick="obtener_datos_editar_alumnos('<?php echo $id_reg; ?>')" data-toggle="modal" data-target="#editarAlumno"><?php echo strtoupper($nombres_apellidos); ?> </a></td>

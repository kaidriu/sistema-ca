<?php
	...
	$resultado = $conexion->query($consulta);
		...
		if($resultado->num_rows > 0 ){			
			...
			$tituloReporte = "Reporte de existencias y consignaciones";
			$titulosColumnas = array('Id producto','Código','Producto','Existencia','Consignación','Total','Medida','Bodega','Marca','USD');
			...
			while ($fila = $resultado->fetch_array()) {
					...
					$suma_entrada=mysqli_query($conexion,"SELECT sum(cant_consignacion) as entradas FROM detalle_consignacion as det_con INNER JOIN encabezado_consignacion as enc_con ON det_con.codigo_unico=enc_con.codigo_unico WHERE det_con.id_producto='".$id_producto."' and det_con.ruc_empresa='".$ruc_empresa."' and enc_con.tipo_consignacion='VENTA' and enc_con.operacion='ENTRADA' ");
					$row_entrada=mysqli_fetch_array($suma_entrada);
					$cantidad_entrada=$row_entrada['entradas'];
					
					
					$suma_devuelta=mysqli_query($conexion,"SELECT sum(cant_consignacion) as devueltas FROM detalle_consignacion as det_con INNER JOIN encabezado_consignacion as enc_con ON det_con.codigo_unico=enc_con.codigo_unico WHERE det_con.id_producto='".$id_producto."' and det_con.ruc_empresa='".$ruc_empresa."' and enc_con.tipo_consignacion='VENTA' and enc_con.operacion='DEVOLUCIÓN' ");
					$row_devuelta=mysqli_fetch_array($suma_devuelta);
					$cantidad_devuelta=$row_devuelta['devueltas'];
					
					$suma_facturada=mysqli_query($conexion,"SELECT sum(cant_consignacion) as facturada FROM detalle_consignacion as det_con INNER JOIN encabezado_consignacion as enc_con ON det_con.codigo_unico=enc_con.codigo_unico WHERE det_con.id_producto='".$id_producto."' and det_con.ruc_empresa='".$ruc_empresa."' and enc_con.tipo_consignacion='VENTA' and enc_con.operacion='FACTURA' ");
					...
					$precio_final=$fila['precio_producto']*($saldo_producto+$saldo_consignado);
			...
			->setCellValue('E'.$i,  $saldo_consignado)
			->setCellValue('F'.$i,  $saldo_producto+$saldo_consignado)
			->setCellValue('G'.$i,  $nombre_medida )
			->setCellValue('H'.$i,  $nombre_bodega)
			->setCellValue('I'.$i,  $nombre_marca)
			->setCellValue('J'.$i,  $precio_final)
			;
						$i++;
			}
			$t=$i+1;
			...
			header('Content-Disposition: attachment;filename="ExistenciasYconsignacion.xlsx"');

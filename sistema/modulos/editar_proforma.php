...
//hacer las consultas a la base de todo el registro de esa proforma a editar
$id_proforma=base64_decode($_GET['id_proforma']);
$busca_datos_encabezados = "SELECT * FROM encabezado_proforma as enc_pro INNER JOIN clientes as cli ON cli.id=enc_pro.id_cliente WHERE enc_pro.id_encabezado_proforma = '".$id_proforma."' ";
...
$nombre_cliente = $datos_encabezados['nombre'];
$ruc = $datos_encabezados['ruc'];
$email = $datos_encabezados['email'];
$telefono = $datos_encabezados['telefono'];
$direccion = $datos_encabezados['direccion'];
$plazo = $datos_encabezados['plazo'];
$total_proforma = $datos_encabezados['total_proforma'];
$tipo_id = $datos_encabezados['tipo_id'];
$codigo_unico = $datos_encabezados['codigo_unico'];
$fecha_proforma = $datos_encabezados['fecha_proforma'];


//datos del cuerpo de la proforma pasa al temporal de proforma
$query_pasa_datos_proforma = mysqli_query($con, "INSERT INTO factura_tmp(id, id_producto, cantidad_tmp, precio_tmp, descuento, tipo_produccion, tarifa_iva, tarifa_ice, tarifa_botellas, id_usuario, id_bodega, id_medida, lote, vencimiento) 
SELECT null,id_producto,cantidad,valor_unitario, descuento,tipo_produccion,tarifa_iva,tarifa_ice,tarifa_bp,'".$id_usuario."',id_bodega,id_medida_salida,lote,vencimiento  FROM cuerpo_proforma WHERE ruc_empresa = '".$ruc_empresa."' and codigo_unico='".$codigo_unico."'");

//para agregar un adicional al temporal de facturas adicional
$query_pasa_detalle_adicional_proforma = mysqli_query($con, "INSERT INTO adicional_tmp (id_ad_tmp, id_usuario, serie_factura, secuencial_factura, concepto, detalle) 
SELECT null, '".$id_usuario."','".$serie."','".$secuencial."',adicional_concepto,adicional_descripcion  FROM detalle_adicional_proforma WHERE ruc_empresa = '".$ruc_empresa."' and codigo_unico='".$codigo_unico."'");
...
<body>
	...
	<div class="container" id="content">
		<div class="panel panel-warning">
			...
			<button id="guardar_datos_proforma" type="submit" class="btn btn-warning btn-md"><span class='glyphicon glyphicon-refresh'></span> Actualizar</button>
			</div>
			<h4><i class='glyphicon glyphicon-edit'></i> Editar proforma</h4>
			</div>
			...
			<div class="well well-sm" style="height:50px;">
					<div class="form-group row">
								...
								<input type="text" class="form-control input-sm" name="fecha_proforma" id="fecha_proforma" value="<?php echo date("d-m-Y", strtotime($fecha_proforma));?>">
								</div>
								</div>
								<div class="col-sm-2">
									<div class="input-group" >
										<span class="input-group-addon"><b>Serie</b></span>
										<input type="text" class="form-control input-sm text-right" id="serie_proforma" name="serie_proforma"  value="<?php echo $serie; ?>" readonly>
									</div>
								</div>
								<div class="col-sm-2">
								<div class="input-group" >
								<span class="input-group-addon"><b>No.</b></span>								
								<input type="text" class="form-control input-sm text-right" id="secuencial_proforma" name="secuencial_proforma"  value="<?php echo $secuencial; ?>" readonly>
								</div>
								</div>
								<div class="col-sm-6">
								  <div class="input-group" >
								  <span class="input-group-addon"><b>Cliente</b></span>
								  <input style="z-index:inherit;" type="text" class="form-control input-sm" id="nombre_cliente_proforma" name="nombre_cliente_proforma" placeholder="Agregue un cliente por ruc o nombre" title="Buscar un cliente." onkeyup='buscar_clientes();' autocomplete="off" value="<?php echo $nombre_cliente; ?>">
								...
								<input type="hidden" id="id_cliente_proforma" name="id_cliente_proforma" value="<?php echo $id_cliente; ?>">
								<input type="hidden" id="tipo_id_cliente" name="tipo_id_cliente" value="<?php echo $tipo_id; ?>">
								<input type="hidden" id="ruc_cliente" name="ruc_cliente" value="<?php echo $nombre_cliente; ?>">
								<input type="hidden" id="telefono_cliente" name="telefono_cliente" value="<?php echo $telefono; ?>">
								<input type="hidden" id="direccion_cliente" name="direccion_cliente" value="<?php echo $direccion; ?>">
								<input type="hidden" id="plazo_credito" name="plazo_credito" value="<?php echo $plazo; ?>">
								<input type="hidden" id="email_cliente" name="email_cliente" value="<?php echo $email; ?>">
								<input type="hidden" id="total_proforma" name="total_proforma" value="<?php echo $total_proforma; ?>">
...
//para guardar el nuevo cliente desde la proforma
/*
$(function() {
							...
							select: function(event, ui) {
								...
								//aqui controla cuando se selecciona producto y trabaja con inventario
									if (tipo_producto=="01" && configuracion_inventario =='SI'){
										...
										var producto = $("#id_producto_agregar").val();
										var codigo_proforma = $("#codigo_unico").val();
									...
									//para que se cargue el stock del producto al momento de buscar el producto dependiendo de la bodega que esta seleeccionada por default
										...
										$.post( '../ajax/saldo_producto_inventario.php', {id_bode: bodega, id_prod: producto, editar_factura: codigo_proforma}).done( function( respuesta_salidas ){
											var saldo_salidas = respuesta_salidas;
...
$(document).ready(function(){
		...
		$.ajax({
			...
			data: "serie_factura_e="+serie_proforma+"&secuencial_factura="+secuencial_proforma+"&id_cliente="+id_cliente_proforma,
			beforeSend: function(objeto){

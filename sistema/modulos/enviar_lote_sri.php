...
<div class="container">
	<div class="panel panel-info">
		<div class="panel-heading">
			...
			<h4><i class='glyphicon glyphicon-search'></i> Documentos pendientes de autorización</h4>	
		</div>
		<div class="panel-body">
				...
				<div class="form-group row">
					...
					<div class="col-md-2">
						...
						<option value="6">Liquidación compras</option>
						</select>
					</div>
					...
					<div class="col-md-2">
						<button type="button" class="btn btn-success" onclick='procesar_comp();'><span class="glyphicon glyphicon-cog" ></span> Procesar  <span class="badge" id="count"></span></button>
...
<script>
  ...
  function search(){
		...
		var count = 0;
		$.ajax({
			url:'../ajax/buscar_comprobantes_pendientes.php?action=buscar_comp&desde='+desde+'&hasta='+hasta+'&documento='+documento,
   ...
   function procesar_comp() {
	...
	if (comp_select =='') {
   	...
   	var r = confirm("Esta seguro que desea procesar los comprobantes seleccionados?");
   	if (r == true) {
   		...
   		if (comp_select!='') {
			...
			$.ajax({
			  type: "POST",
			  dataType: "html",
			  url: '../ajax/buscar_comprobantes_pendientes.php?action=procesar_comp',
			  data: {comp_select:comp_select, documento:documento},
				 
			 beforeSend: function(){			
				$('#loader').html(	'<div class="progress"><div class="progress-bar progress-bar-success progress-bar-striped active" role="progressbar" style="width:100%;">Enviando documentos, espere por favor...</div></div>');
			 }, 

...
public function lee_clave_acceso($clave, $ruc_empresa, $id_usuario, $con){
	...
	if (is_object($object_xml)) {
		...
		$nombre_proveedor = $object_xml->infoLiquidacionCompra->razonSocialProveedor;
			...
			if (substr($ruc_comprador,0,12)==substr($ruc_empresa,0,12)){//para ver si quien nos emite la lc nos emite a esta empresa
				$id_proveedor =	$this->proveedor_cliente("proveedor", $con, $ruc_empresa, $object_xml, $ruc_proveedor);
					...
					$id_proveedor =	$this->proveedor_cliente("proveedor", $con, $ruc_empresa, $object_xml, $ruc_proveedor);
					$registros_compras = comprueba_registrados($con, $ruc_empresa, 'lc_compra', $serie, $secuencial, $id_proveedor);
		...
		if (substr($ruc_comprador,0,12)==substr($ruc_empresa,0,12)){//para ver si quien nos emite la factura nos emite a esta empresa
			$id_proveedor =	$this->proveedor_cliente("proveedor", $con, $ruc_empresa, $object_xml, $ruc_proveedor);
				$registros = comprueba_registrados($con, $ruc_empresa, 'nc_compra', $serie, $secuencial, $id_proveedor);
				if ($registros==0){
		...
		if (substr($ruc_comprador,0,12)==substr($ruc_empresa,0,12)){//para ver si quien nos emite la nd nos emite a esta empresa
			$id_proveedor =	$this->proveedor_cliente("proveedor", $con, $ruc_empresa, $object_xml, $ruc_proveedor);
				$registros = comprueba_registrados($con, $ruc_empresa, 'nd_compra', $serie, $secuencial, $id_proveedor);
				if ($registros==0){
...
public function proveedor_cliente($tipo, $con, $ruc_empresa, $object_xml, $ruc_proveedor){
	...
	if ($tipo=="cliente"){
		...
		$sql_guarda_cliente=mysqli_query($con, "INSERT INTO clientes VALUES (null,'".$ruc_empresa."','".$razon_social."','04','".$ruc_proveedor."', '".$telefono_proveedor_cliente."','".$mail_proveedor_cliente."','".$direccion_proveedor_cliente."','1', '".$fecha_registro."','1','".$id_usuario."','17','189')");
...
function comprueba_registrados($con, $ruc_empresa, $registrar_en, $serie, $secuencial, $id_proveedor_cliente){
	switch ($registrar_en) {
			case "factura_compra"://para guardar facturas, nc y liquidaciones en las compras
				$sql_encabezado=mysqli_query($con,"SELECT count(*) as total FROM encabezado_compra WHERE mid(ruc_empresa,1,12) = '".substr($ruc_empresa,0,12)."' and numero_documento = '".$serie."-".$secuencial."' and id_proveedor='".$id_proveedor_cliente."' and id_comprobante ='1' ");	
				break;
			case "nc_compra"://para guardar facturas, nc y liquidaciones en las compras
					$sql_encabezado=mysqli_query($con,"SELECT count(*) as total FROM encabezado_compra WHERE mid(ruc_empresa,1,12) = '".substr($ruc_empresa,0,12)."' and numero_documento = '".$serie."-".$secuencial."' and id_proveedor='".$id_proveedor_cliente."' and id_comprobante ='4' ");	
					break;
			case "lc_compra"://para guardar facturas, nc y liquidaciones en las compras
				$sql_encabezado=mysqli_query($con,"SELECT count(*) as total FROM encabezado_compra WHERE mid(ruc_empresa,1,12) = '".substr($ruc_empresa,0,12)."' and numero_documento = '".$serie."-".$secuencial."' and id_proveedor='".$id_proveedor_cliente."' and id_comprobante = '7' ");	
				break;
			case "nd_compra"://para guardar facturas, nc y liquidaciones en las compras
				$sql_encabezado=mysqli_query($con,"SELECT count(*) as total FROM encabezado_compra WHERE mid(ruc_empresa,1,12) = '".substr($ruc_empresa,0,12)."' and numero_documento = '".$serie."-".$secuencial."' and id_proveedor='".$id_proveedor_cliente."' and id_comprobante = '5' ");	
				break;

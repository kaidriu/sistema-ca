<?php
$configuracion_facturacion = new guarda_info_bd();
...
if (isset($_POST['guarda_aplica_inventario'])){
			...
			$consulta_configuracion=$configuracion_facturacion->consulta_configuracion_facturacion($id_confi, $con);	
			if(isset($id_confi) && !empty($id_confi)){
			echo $configuracion_facturacion->actualiza_configuracion_facturacion($id_confi, $serie_sucursal, $consulta_configuracion['propina'], $consulta_configuracion['tasa_turistica'], $consulta_configuracion['clientes'], $consulta_configuracion['productos'], $consulta_configuracion['medida'], $consulta_configuracion['lote'], $consulta_configuracion['bodega'], $consulta_configuracion['vencimiento'], $consulta_configuracion['lote_impreso'], $consulta_configuracion['medida_impreso'], $consulta_configuracion['bodega_impreso'], $consulta_configuracion['vencimiento_impreso'], $opcion_trabaja_inventario, $consulta_configuracion['calculo_salida'],  $con);	
			}else{
			echo $configuracion_facturacion->guarda_configuracion_facturacion($serie_sucursal, '', '', '', '', '', '', '', '', '', '', '','', $opcion_trabaja_inventario,'',  $con);
			} 
			}else {
...
//para guardar o editar propina tasa
if (isset($_POST['guarda_propina_tasa'])){
			if (empty($_POST['serie_sucursal_propina_tasa'])) {
			   echo "<script> $.notify('Seleccione una sucursal.','error')</script>";
			}else if (empty($_POST['aplica_propina'])) {
				echo "<script> $.notify('Seleccione si desea mostrar la casilla, propina.','error')</script>";
			}else if (empty($_POST['aplica_tasa'])) {
				echo "<script> $.notify('Seleccione si desea mostrar la casilla, tasa.','error')</script>";
			} else if (!empty($_POST['serie_sucursal_propina_tasa'])&& !empty($_POST['aplica_propina']) && !empty($_POST['aplica_tasa'])){
			$id_confi=$_POST["id_conf_propina_tasa"];
			$serie_sucursal=mysqli_real_escape_string($con,(strip_tags($_POST["serie_sucursal_propina_tasa"],ENT_QUOTES)));
			$aplica_propina=mysqli_real_escape_string($con,(strip_tags($_POST["aplica_propina"],ENT_QUOTES)));
			$aplica_tasa=mysqli_real_escape_string($con,(strip_tags($_POST["aplica_tasa"],ENT_QUOTES)));
			$consulta_configuracion=$configuracion_facturacion->consulta_configuracion_facturacion($id_confi, $con);
			if(isset($id_confi) && !empty($id_confi)){
			echo $configuracion_facturacion->actualiza_configuracion_facturacion($id_confi, $serie_sucursal, $aplica_propina, $aplica_tasa, $consulta_configuracion['clientes'], $consulta_configuracion['productos'], $consulta_configuracion['medida'], $consulta_configuracion['lote'], $consulta_configuracion['bodega'], $consulta_configuracion['vencimiento'], $consulta_configuracion['lote_impreso'], $consulta_configuracion['medida_impreso'], $consulta_configuracion['bodega_impreso'], $consulta_configuracion['vencimiento_impreso'], $consulta_configuracion['inventario'], $consulta_configuracion['calculo_salida'], $con);	
			}else{
			echo $configuracion_facturacion->guarda_configuracion_facturacion($serie_sucursal, $aplica_propina, $aplica_tasa, '', '', '', '', '', '', '', '', '','', '','',  $con);
			} 
			}else {
			   echo "<script> $.notify('Error desconocido.','error')</script>";	
			}
...
if (isset($_POST['guarda_productos_clientes'])){
			...
			$consulta_configuracion=$configuracion_facturacion->consulta_configuracion_facturacion($id_confi, $con);
			if(isset($id_confi) && !empty($id_confi)){
			echo $configuracion_facturacion->actualiza_configuracion_facturacion($id_confi, $serie_sucursal, $consulta_configuracion['propina'], $consulta_configuracion['tasa_turistica'], $compartir_clientes, $compartir_productos, $consulta_configuracion['medida'], $consulta_configuracion['lote'], $consulta_configuracion['bodega'], $consulta_configuracion['vencimiento'], $consulta_configuracion['lote_impreso'], $consulta_configuracion['medida_impreso'], $consulta_configuracion['bodega_impreso'], $consulta_configuracion['vencimiento_impreso'], $consulta_configuracion['inventario'], $consulta_configuracion['calculo_salida'], $con);	
			}else{
			echo $configuracion_facturacion->guarda_configuracion_facturacion($serie_sucursal, '', '', $compartir_clientes, $compartir_productos, '', '', '', '', '', '', '','', '','',  $con);
			} 
			}else {
			   echo "<script> $.notify('Error desconocido.','error')</script>";	
			}
...
if (isset($_POST['guarda_lista_opciones_inventarios'])){
			...
			$consulta_configuracion=$configuracion_facturacion->consulta_configuracion_facturacion($id_confi, $con);
			if(isset($id_confi) && !empty($id_confi)){
			echo $configuracion_facturacion->actualiza_configuracion_facturacion($id_confi, $serie_sucursal, $consulta_configuracion['propina'], $consulta_configuracion['tasa_turistica'], $consulta_configuracion['clientes'], $consulta_configuracion['productos'], $mostrar_medida, $mostrar_lote, $mostrar_bodega, $mostrar_caducidad, $consulta_configuracion['lote_impreso'], $consulta_configuracion['medida_impreso'], $consulta_configuracion['bodega_impreso'], $consulta_configuracion['vencimiento_impreso'], $consulta_configuracion['inventario'], $consulta_configuracion['calculo_salida'], $con);	
			}else{
			echo $configuracion_facturacion->guarda_configuracion_facturacion($serie_sucursal, '', '', '', '', $mostrar_medida, $mostrar_lote, $mostrar_bodega, $mostrar_caducidad, '', '', '','', '','', $con);
			} 
			}else {
			   echo "<script> $.notify('Error desconocido.','error')</script>";	
			}
...
if (isset($_POST['guarda_imprime_etiquetas'])){
			...
			echo $configuracion_facturacion->guarda_configuracion_facturacion($serie_sucursal, '', '', '', '', '', '', '', '', $mostrar_lote_impresion, $mostrar_medida_impresion, $mostrar_bodega_impresion, $mostrar_caducidad_impresion, '','',  $con);
			} 
			}else {
			   echo "<script> $.notify('Error desconocido.','error')</script>";	
			}
...
if (isset($_POST['guarda_salidas_inventario'])){
			...
			echo $configuracion_facturacion->guarda_configuracion_facturacion($serie_sucursal, '', '', '', '', '', '', '', '', '', '', '', '', '', $tipo_salida_inventario, $con);
			} 
			}else {
			   echo "<script> $.notify('Error desconocido.','error')</script>";	
			}
...
//para consultar cuando se seleccione una sucursal en cada una de las opciones de configuracion de facturacion
	if (isset($_POST['opcion_consulta'])){
		...
		$sql = mysqli_query($con,"SELECT * FROM configuracion_facturacion where ruc_empresa ='".$ruc_empresa."' and serie_sucursal ='".$serie_sucursal."';");
		$info_configuracion = mysqli_fetch_array($sql);
		$id_conf = $info_configuracion['id_confi'];
		$inventario = $info_configuracion['inventario'];
		$propina = $info_configuracion['propina'];
		$tasa = $info_configuracion['tasa_turistica'];
		...
		$calculo_salida = $info_configuracion['calculo_salida'];
			...
			switch ($opcion_consulta) {
				...
				case "propina_tasa":
					?>
					<input type="hidden" value="<?php echo $id_conf;?>" id="id_configuracion">
					<input type="hidden" value="<?php echo $propina;?>" id="propina">
					<input type="hidden" value="<?php echo $tasa;?>" id="tasa">
					<?php
					break;
...
class guarda_info_bd{
	///para guardar en la tabla configuracion_facturacion
		public function guarda_configuracion_facturacion($serie_sucursal, $propina, $tasa, $clientes, $productos, $medida, $lote, $bodega, $vencimiento, $lote_impreso, $medida_impreso, $bodega_impreso, $vencimiento_impreso, $inventario,$calculo_salida, $con){
		//session_start();
		$ruc_empresa = $_SESSION['ruc_empresa'];   
 		$guarda_configuracion = mysqli_query($con, "INSERT INTO configuracion_facturacion VALUES (null, '".$ruc_empresa."', '".$serie_sucursal."', '".$propina."', '".$tasa."', '".$clientes."', '".$productos."', '".$medida."', '".$lote."', '".$bodega."', '".$vencimiento."', '".$lote_impreso."', '".$medida_impreso."', '".$bodega_impreso."', '".$vencimiento_impreso."', '".$inventario."','".$calculo_salida."')");
		...
		///para actualizar en la tabla configuracion_facturacion
		public function actualiza_configuracion_facturacion($id_confi, $serie_sucursal, $propina, $tasa, $clientes, $productos, $medida, $lote, $bodega, $vencimiento, $lote_impreso, $medida_impreso, $bodega_impreso, $vencimiento_impreso, $inventario, $calculo_salida, $con){
		$actualiza_configuracion = mysqli_query($con, "UPDATE configuracion_facturacion SET serie_sucursal='".$serie_sucursal."', propina='".$propina."', tasa_turistica = '".$tasa."', clientes='".$clientes."', productos='".$productos."', medida='".$medida."', lote='".$lote."', bodega='".$bodega."', vencimiento='".$vencimiento."', lote_impreso='".$lote_impreso."', medida_impreso='".$medida_impreso."', bodega_impreso='".$bodega_impreso."', vencimiento_impreso='".$vencimiento_impreso."', inventario='".$inventario."', calculo_salida='".$calculo_salida."' WHERE id_confi='".$id_confi."' ");
		...
		public function consulta_configuracion_facturacion($id_confi, $con){	
		$consulta_configuracion = mysqli_query($con, "SELECT * FROM configuracion_facturacion WHERE id_confi='".$id_confi."' ");
